...
class Vta1Dataset():
    def __init__(self, treatment, task, session=None):
    ...
    def spike_times_per_trial(self, odor):
        ...
        '''
        trial_type_map = {"A": list(range(0,10)), "B": [10], "C": [11]}
        time_index = self.times.reset_index()
        Task1_index = time_index[time_index["task"] == self.task]
        Saline_index = Task1_index[Task1_index["treatment"] == self.treatment]
        sessionarray = Saline_index["session"].unique()
        spikes_trial = []
        isi_list = []
        isi_map = {float(trial_type): 1.2 + (trial_type) * 0.2 for trial_type in range(10)}
        isi_map[10.0] = 1.2 
        isi_map[11.0] = 2.8
        for session in sessionarray:
            task1_sal_odor=self.times.loc[self.treatment, self.task, session]
            task1_sal_spikes=self.spikes.loc[self.treatment, self.task, session]
            odor_table = task1_sal_odor[task1_sal_odor["trial_type"].isin(trial_type_map[odor])]
            for _,row in odor_table.iterrows():
                start = row["odor_onsets"]
                end = start + self.window*1000 
                isi = isi_map[row["trial_type"]]
                trial_isi=task1_sal_spikes[(task1_sal_spikes["spiketimes"]>= start) & (task1_sal_spikes["spiketimes"] <= end)]
    ...
    def binned_spike_counts_per_trial(self, odor, bin_count = 50):
        ...
        hist_odor = []
        spikes_trial, isi_list = self.spike_times_per_trial(odor)
        for interval, isi in zip(spikes_trial, isi_list):   
            T = self.window*1000
            bin_size = T / bin_count
            bins = np.arange(0, T + bin_size, bin_size)
            spikes_t = interval.to_numpy()
            r_est, edges  = np.histogram(spikes_t, bins=bins)
